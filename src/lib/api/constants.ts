 ...
 * which on a device is the phone itself (every request fails -> "server_error"
 * + empty screens). Used when EXPO_PUBLIC_API_URL wasn't baked into the build.
 */
...
function resolveBaseUrl(): string {
  ...
  if (fromEnv) {
    ...
    // A released build must NEVER point at localhost (= the device itself ->
    // "server_error" + empty screens). The local .env pins localhost and gets
    // baked into a manual/Xcode build, so override it with the production API.
  ...
  // No env var baked in. A released build (__DEV__ === false) has no Expo dev
  // server, so it must use the production API rather than falling back to
  // localhost — that fallback caused "server_error" + an empty home on TestFlight.
